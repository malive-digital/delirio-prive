...
export async function generateMetadata({ params }: CategoryCityPageProps): Promise<Metadata> {
  ...
  const description = `Encontre acompanhantes mulheres em ${cityTitle} no Delirio Prive.`;
  const path = getCatalogCategoryCityHref(category, city);

  return {
    title: `Mulheres em ${cityTitle} | Delirio Prive`,
    description,
    alternates: {
      canonical: `${siteUrl}${path}`,
    },
    openGraph: {
      title: `Mulheres em ${cityTitle}`,
      description,
      url: `${siteUrl}${path}`,
...
export default async function MulheresCityPage({ params }: CategoryCityPageProps) {
  const { city } = await params;
  const { profile } = await getCategoryCityProfile(city);
