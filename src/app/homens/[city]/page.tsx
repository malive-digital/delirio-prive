...
const category = CATALOG_CATEGORIES[1];
...
export async function generateMetadata({ params }: CategoryCityPageProps): Promise<Metadata> {
  ...
  const description = `Encontre acompanhantes homens em ${cityTitle} no Delirio Prive.`;
  const path = getCatalogCategoryCityHref(category, city);

  return {
    title: `Homens em ${cityTitle} | Delirio Prive`,
    description,
    alternates: {
      canonical: `${siteUrl}${path}`,
    },
    openGraph: {
      title: `Homens em ${cityTitle}`,
      description,
      url: `${siteUrl}${path}`,
...
export default async function HomensCityPage({ params }: CategoryCityPageProps) {
  const { city } = await params;
  const { profile } = await getCategoryCityProfile(city);
