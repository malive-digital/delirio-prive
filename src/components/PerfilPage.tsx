...
export default function PerfilPage({ citySlug = "", profileSlug: routeProfileSlug = "" }: PerfilPageProps) {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const profileSlug = routeProfileSlug || searchParams.get("modelo");
      const legacyProfileId = searchParams.get("id");

      if (!profileSlug && !legacyProfileId) {
      ...
      const profileQuery = supabase
        ...
        .select("id,type,name,whatsapp,location,state_uf,headline,age,neighborhood,price_15,price_30,price_60,overnight_price,serves,has_place,availability,payment_methods,services,specialties,restrictions,appearance,languages,description,active_plan,is_online,profile_verified,created_at");

      const { data: profileData, error: profileError } = legacyProfileId
        ? await profileQuery.eq("id", legacyProfileId).maybeSingle()
        : await profileQuery.then(({ data, error }) => ({
            data: (data || []).find((item) => {
              if (citySlug) {
                return buildProfileCitySlug(item) === citySlug && buildProfilePath(item, data || []).split("/").pop() === profileSlug;
              }

              return buildProfilePath(item, data || []).split("/").pop() === profileSlug || buildLegacyProfileSlug(item) === profileSlug;
            }) || null,
            error,
          }));
  ...
  const safeSelectedPhotoIndex = photos.length ? Math.min(selectedPhotoIndex, photos.length - 1) : 0;
                ...
                {photos.length > 1 && (
                  <div className="gallery-preview-grid" aria-label="Fotos aprovadas">
                    {photos.map((photo, index) => (
                      <button
                        className={`gallery-thumb ${safeSelectedPhotoIndex === index ? "is-active" : ""}`}
                        key={photo}
                        type="button"
